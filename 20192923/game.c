...
void LoadRanking(RANKING* head); // 외부 파일에 있는 랭킹을 불러오는 함수
int main()
{
	LoadRanking(list_head); // 실행하면 랭킹 바로 불러오기
	mainMenu(); // 메인 메뉴로 가는 함수
}
...
{
	printf("=는 게임판(5x5), O는 플레이어, X는 적입니다.\n\n");
	printf("플레이어는 적을 피해서 도망다녀야 합니다.\n\n");
	printf("적은 상하좌우로 한칸 움직일 수 있고, 움직이지 않을수도 있습니다. 플레이어는 상하좌우로만 한칸 움직일 수 있습니다.\n\n");
	printf("한 위치에 여러마리의 적이 있을 수 있습니다.\n\n");
	printf("플레이어가 먼저 움직이고, 적이 움직입니다. 각자 한번씩 움직이면 한턴이 끝나면서 플레이어는 점수를 얻습니다.\n\n");
	printf("난이도에 따라 턴마다 얻는 점수가 다릅니다.\n\n");
	printf("적의 적진(+)에 가면 보너스 점수 5점을 얻습니다.\n\n");
...
void GameStart(int level, int (*func)(int)) // 게임 시작
{
	...
	while (1) {
		...
		if (outWhile)
			break;
		if (player.row == bonus.row && player.col == bonus.col) {
			printf("적의 본진을 점령했습니다!! +5점\n");
			score += 5;
			BonusMove(); // 보너스 발판을 랜덤으로 다시 설정
		}
		printf("적들이 움직일 차례입니다.\n");
		usleep(500000); // 0.5초 시간지연 
		EnemyMove(); // 적 움직임
...
void PrintGameBoard()
{
	...
	strcpy(gameBoard[bonus.row][bonus.col], "+"); // 보너스 발판 위치 색칠

	strcpy(gameBoard[player.row][player.col], "O"); // 플레이어 위치 o으로 색칠

	for (int i = 0; i < enemyCount; i++)
	{
		strcpy(gameBoard[enemys[i].row][enemys[i].col], "X"); // 적들 위치 ㅁ으로 색칠
	}
...
void InitializationBoard() // 게임판 =로 초기화
{
	for (int i = 0; i < size; i++)
	{
		for (int j = 0; j < size; j++)
